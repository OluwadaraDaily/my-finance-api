...
def test_get_transactions(db_session, test_transaction_data, test_user, test_category):
    ...
    transactions = service.get_transactions(account_id=test_transaction_data["account_id"])
    ...
    transactions = service.get_transactions(account_id=test_transaction_data["account_id"], filters=filters)
    assert len(transactions) == 1
    assert transactions[0].amount == 2000
    assert transactions[0].type == TransactionType.CREDIT

def test_get_transaction_by_id(db_session, test_user, test_account, test_category):
    service = TransactionService(db_session)
...
def test_update_transaction_with_account_update(db_session, test_user, test_account, test_category):
    service = TransactionService(db_session)
    ...
    assert test_account.balance == initial_balance + 1000 + 1500

def test_delete_transaction_with_account_update(db_session, test_user, test_account, test_category):
    service = TransactionService(db_session)
    ...
    result = service.delete_transaction(transaction.id, test_account.id)
    db_session.refresh(test_account)
    
    assert result is True
    # Original debit of 1000 should be reverted
    assert test_account.balance == initial_balance + 1000
...
def test_get_transaction_summary(db_session, test_user, test_account, test_category):
    service = TransactionService(db_session)
    ...
    summary = service.get_transaction_summary(account_id=test_account.id)
